...
@Singleton
class WeatherDataRepositoryImpl @Inject()(db: Database)(implicit ec: ExecutionContext) extends WeatherDataRepository[IO]{

    //Получение данных о погоде по локации
    override def getWeatherByLocation(location: String): IO[Seq[WeatherModel]] = IO.blocking {
    ...
    override def getAllWeather(): IO[Seq[WeatherModel]] = IO.blocking {
        db.withConnection{conn =>
    ...
    override def getLatestWeather(locations: List[String], services: List[String]): IO[Seq[WeatherModel]] = IO.blocking {
        db.withConnection { conn =>
            val buffer = Buffer[WeatherModel]()
            for {
                location <- locations
    ...
    override def insert(weather: WeatherModel): IO[WeatherModel] = IO.blocking {
        db.withConnection{conn =>
            val stmt = conn.prepareStatement(
    ...
    override def delete(id: Int): IO[Unit] = IO.blocking {
        db.withConnection{conn =>
