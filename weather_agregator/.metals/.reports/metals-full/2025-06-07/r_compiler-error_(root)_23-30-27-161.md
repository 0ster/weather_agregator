...
### java.nio.file.InvalidPathException: Illegal char <:> at index 3: jar:file:///C:/Users/USER/AppData/Local/Coursier/cache/v1/https/repo1.maven.org/maven2/org/scala-lang/scala-library/2.13.16/scala-library-2.13.16-sources.jar!/scala/collection/immutable/LazyList.scala
...
class WeatherDataServiceImpl[F[_]: Async] @Inject()(repository: WeatherDataRepository[F], config: Configuration, ws: WSClient)(implicit ec: ExecutionContext, materializer: Materializer) extends WeatherDataService[F] {
    ...
    val interval = 2.minutes
     ...
     def startPeriodicDataCollection(): F[Unit] = {
        // Бесконечный цикл городов
        val citiesCycle: LazyList[String] = LazyList.continually(locations).flatten
        
        // Связываем каждый город с задержкой в 2 минуты
        val process: LazyList[F[Unit]] = citiesCycle.m@@(LazyList.continually(interval))
        .mapEval { case (location, delay) =>
            for {
            _ <- F.delay(println(s"Fetching data for $location at ${LocalDateTime.now()}"))
            _ <- fetchFromAllServices(location) // Запрашиваем данные от всех сервисов
            _ <- F.sleep(delay)                // Ждем указанный интервал
            } yield ()
        }
        
        // Запускаем процесс в фоне
        F.delay(process.compile.drain.unsafeRunAsync(_ => ()))
    }    

     // Запрос данных от всех сервисов для одного города
    private def fetchFromAllServices(location: String): F[Unit] = {
        services.traverse_ { service =>
            fetchWeather(service, location)
            .flatMap(insert)
            .handleErrorWith(e => 
                F.delay(println(s"Error fetching $service data for $location: ${e.getMessage}")))
        }
    }
