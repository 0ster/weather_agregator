...
### dotty.tools.dotc.core.MissingType: Cannot resolve reference to type Services.type.WeatherDataService.
...
class WeatherDataServiceImpl @Inject()(repository: WeatherDataRepository[IO], config: Configuration, ws: WSClient)(implicit ec: ExecutionContext, materializer: Materializer) extends WeatherDataService[IO] {
    ...
    override def startPeriodicDataCollection(): IO[Unit] = {
        ...
        // Функция, которая запускает эффекты один за другим
        def loop(stream: LazyList[IO[Unit]]): IO[Unit] = {
            stream match {
            case head #:: tail =>
                head.flatMap(_ => loop(tail))
            case _ => IO.unit 
            }
        }

        Con@@IO.start(loop(process)).void
    }

    //Агрегация данных от нескольких сервисов
    override def fetchFromAllServices():IO[Seq[WeatherModel]] = {
        locations.traverse{location =>
            services.traverseFilter{service =>
                fetchWeather(service, location)
                    .flatMap{model =>
                        insert(model).map(_ => Some(model))
                    }
                    .handleErrorWith{e =>
                        IO.delay{
                            println(s"Ошибка получения данных из $service для $location: ${e.getMessage}")
                            None
                        }
                    }
            }
